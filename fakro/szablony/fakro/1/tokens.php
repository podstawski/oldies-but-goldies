...
function tokens($t)
{
  global $WEBPAGE, $KAMELEON_MODE, $UIMAGES, $UFILES, $WEBTD, $WEBLINK, $IMAGES, $KAMELEON_UIMAGES, $INCLUDE_PATH;
    ...
    case "WEBPAGE_TITLE_MAIN":
      ...
      for($i=count($pages_list); $i >= 0 ; $i--) {
        if(strlen($pages_list[$i])) {
          ...
          if(strlen($PREV_WEBPAGE[0]->title) && ($PREV_WEBPAGE[0]->hidden==0 && $PREV_WEBPAGE[0]->nositemap==0))
            $title[] = ", ".$PREV_WEBPAGE[0]->title;
        }
      }
      
      $title_reverse = array_reverse($title);
      
      $res="";
      foreach($title_reverse as $title) {
        if(!preg_match("(new page|nowa strona)i", $title)) $res .= $title;
      }
      $res = substr($res,2);
...
//      if (!strlen($WEBPAGE->class)) 
    ...
    case "WEBPAGE_CSS_MAIN":
      global $CONST_MORE_CSS_MAIN;
      
      if(is_array($CONST_MORE_CSS_MAIN)) {
        $scr="";
        for($i=1; $i<=count($CONST_MORE_CSS_MAIN); $i++) {
          $scr .= "\n\t<link href=\"$IMAGES/$CONST_MORE_CSS_MAIN[$i]\" rel=\"stylesheet\" type=\"text/css\">";
          }
        }
      return "$scr";
    break; 
    ...
    case "WEBPAGE_JS_MAIN":
      global $CONST_MORE_JS_MAIN;
      
      if(is_array($CONST_MORE_JS_MAIN)) {
        $scr="";
        for($i=1; $i<=count($CONST_MORE_JS_MAIN); $i++) {
          $scr .= "\n\t<script src=\"$IMAGES/$CONST_MORE_JS_MAIN[$i]\" type=\"text/javascript\"></script>";
          }
        }
      return "$scr";
    break;
...
/*************************************************************************/
    case "WEBLINK_GALERY":
      ...
      if ($WEBLINK->img) {
        ...
        $_abegin="<a href=\"$UIMAGES/$WEBLINK->imga\" class=\"zoom\" rel=\"group\">";
        $_aend = "</a>";
...
/**/
    ...
    case "WEBLINK_GALERY_ENDROW":  
      ...
      /////////////////////////////////////////////////////////////////////////////////////////////////////////////
    case "WEBLINK_KATALOG":
      $_kat = ' ';

      if($WEBLINK->img && $WEBLINK->page_target && $WEBLINK->ufile_target) {
        $_src = $KAMELEON_UIMAGES."/".$WEBLINK->img;
        ...
        if($WEBLINK->page_target != -1) $link = kameleon_href('','',$WEBLINK->page_target);
        else $link = $WEBLINK->href;
		
        $_kat = '<span class="img"><img src="' . $UIMAGES . '/' . $WEBLINK->img . '" border="0" alt="' . strip_tags($WEBLINK->alt) . '" ' . $_size[3].$_class . '></span>';
        $_kat .= '<span class="blok">';
        $_kat .= '<span class="title"><strong>' . strip_tags($WEBLINK->alt) . '</strong></span>';
        $_kat .= '<span class="link"><h3><a href="' . $UFILES . '/' . $WEBLINK->ufile_target .'" target="' . $WEBTD->katalog_cel . '">' . $WEBTD->katalog_link_folder . '</a></h3></span>';
        $_kat .= '<span class="link"><h3><a href="' . $link .'">' . $WEBTD->katalog_link_wiecej . '</a></h3></span>';
        $_kat .= '</span>';
      }
    ...
    case "WEBLINK_KATALOG_CLASS":
      if($WEBTD->katalog_kolumny) $_link_maxcol = $WEBTD->katalog_kolumny;
        else $_link_maxcol = 2;

      return ' class="kat'.$_link_maxcol.'" ';
      break;

    case "WEBLINK_KATALOG_ENDROW":
      global $_link_count_row, $_menu_id;
    ...
    case "SID_GOOGLE_CODE":
      $PREV_WEBPAGE = kameleon_page(0);
      if($PREV_WEBPAGE[0]->pagekey) return '<meta name="google-site-verification" content="'.$PREV_WEBPAGE[0]->pagekey.'">';
    ...
    case "FLASH_IPAD":
      ...
      $re = '';
      if($WEBTD->link_img_ipad) {
        $_src = $KAMELEON_UIMAGES."/".$WEBTD->link_img_ipad;
        if(file_exists($_src)) {
          $_size = getimagesize($_src);
          ...
          <!--
            jQuery(function(){
              ...
              if(isiPad === false) {
                $(\"#ta_img_".$WEBTD->sid."\").remove();
              }else{
                $(\"#ta_img_".$WEBTD->sid."\").show()
                $(\"#s".$WEBTD->sid." object\").remove();
              }
            })
          // -->
          </script>";
          ...
          $re .= $page_script;
          $re .= ' <span id="ta_img_'.$WEBTD->sid.'" style="display: none;">';
          
          if($WEBTD->link_href_ipad) {
            $link_href_ipad = kameleon_href('','',$WEBTD->link_href_ipad);
            $re .= ' <a href="'.$link_href_ipad.'">';
          }
          $re .= '<img '.$_size[3].' border="0" src="'.$UIMAGES."/".$WEBTD->link_img_ipad.'" alt="">';
          if($WEBTD->link_href_ipad) $re .= '</a>';
