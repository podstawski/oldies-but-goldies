<?
	...
	for ($i=0; $i < $res->RecordCount(); $i++)
	{
		...
		if (!$CIACHO[kontrahent_id] || $rodzaj==$za_status)
		{
			if (strlen($up))
		...
		$printbtn = "<a href=\"$self${next_char}action=PDF&list[pdf]=zamowienie&list[prn]=1$_more&list[id]=$za_id&list[show_indx]=1\" target=\"pdf\"><img src=\"$SKLEP_IMAGES/i_acroread.gif\" hspace=2 alt=\"$altp\" border=0></a>";
...
//		$buttons.=" &nbsp; ".$printbtn;
		...
		if (strstr($platnosc,"art"))
			$rodzaj_p = "<br><img src=\"$IMAGES/visa.gif\" border=\"0\">";
		else
			$rodzaj_p = "";
