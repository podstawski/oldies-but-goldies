...
function search()
{
	...
	$smarty->assign('title', 'Zamówienie części');
...
function results()
{
    ...
    $smarty->assign('title', 'Zamówienie cześci');
	$smarty->assign('zglaszajacy_caption','Dane do faktury');
	$smarty->assign('adres_zamontowania_caption','Adres dostarczenia przesyłki');
...
function details()
{
    ...
    foreach ($_POST['ilosc'] as $id_bp_czesci => $ilosc)
	{
		if ($ilosc > 0)
		{
			...
			$dodatkowyKomentarz .= ' '.$_POST['czesci'][$id_bp_czesci] . ' (Ilość: '.$ilosc.', ' .
