<?
$from_name = "PPP - Pomysl o przyszlosci";
$from_mail = "zawody@pomysloprzyszlosci.org";
$data_zawody = '2012';

/* konta mail`e */
$mail_do_firmy["organizator"] = "zawody@pomysloprzyszlosci.org";
?>
<?
$idb->query("SELECT count(*) AS ile FROM zawody_zgloszenie_ppp");
$max = $idb->getvalues();

$datastop = date("Ymd");

if($max['ile'] >= 120 || $datastop >= "20120218") {
# blokada
?>
<br><br>
...
<tr>
	...
	<dd>Przepraszamy, ale lista zgłoszeń do I Narciarskich Mistrzostw Polskich Pracodawców Prywatnych została zamknięta. 
...
$idb->query('SELECT count(*) AS ile FROM zawody_zgloszenie_ppp WHERE nip = "'.$nip_replace.'"');
$count = $idb->getvalues();
$count_nip = $count['ile'];
...
if($_POST[poczta] != '') {
	...
	if(trim($form[telefon]) == '' || trim($form[email]) == '') $errors = 1;
...
<tr>
	<td align="right"></td>
	<td colspan="2">* proszę zakreślić czy w chwili dokonywania zgłoszenia przesłana została opłata startowa w wysokości 200 zł (zgodnie z regulaminem zawodów).</td>
</tr>
</table>
...
<tr>
	...
	I Narciarskich Mistrzostw Polskich Pracodawców Prywatnych - zgodnie z ustawą z dnia 29 sierpnia 1997 roku o ochronie danych osobowych
...
<tr>
	...
	<td><div align="justify"><font style="color: #<?=$oswiadczenief;?>">Oświadczam, iż zapoznałam(em) się z Regulaminem I Narciarskich Mistrzostw Polskich Pracodawców Prywatnych.</font></div></td>
</tr>
</table>
<table width="550" border="0" cellspacing="3" cellpadding="3">
<tr>
...
<?
	...
	$error = $idb->query('INSERT INTO zawody_zgloszenie_ppp SET
	...
	if(!$idb->error_number($error)) {
		...
		$header['From'] = "PPP - Pomysl o przyszlosci <".$from_mail.">";
		$header['To'] = $mail_do_firmy["organizator"];
		$header['Reply-to'] = $from_mail;
		$header['Subject'] = "zgloszenie ppp.pomysloprzyszlosci.org - mail ".date("Y-m-d H:i:s", time())." - ".$form[firma];
