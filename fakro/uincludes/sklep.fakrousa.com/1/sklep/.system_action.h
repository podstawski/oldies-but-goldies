<?
	...
	else 
	{
		...
		if ($prev!=$so_parent || stripslashes($so_nazwa)!=stripslashes($nazwa))
		{
			...
			$query="UPDATE system_obiekt SET so_parent=$prev, so_nazwa='$nazwa' WHERE so_id=$so_id;
				DELETE FROM system_acl_obiekt WHERE sao_server=$SERVER_ID AND sao_klucz='p_$page';
				INSERT INTO system_acl_obiekt (sao_grupa_id,sao_server,sao_klucz)
				SELECT sao_grupa_id,sao_server,'p_$page' FROM system_acl_obiekt WHERE sao_klucz='p_$prev' AND sao_server=$SERVER_ID";
			parse_str(query2url($query));
		}
