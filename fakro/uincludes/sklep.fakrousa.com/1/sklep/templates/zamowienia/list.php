<?
	...
	/*FAKRO*/
	$query = "SELECT za_poczta_br,tr_strefa_vat FROM zamowienia WHERE za_id = $za_id";
	parse_str(ado_query2url($query));
	/*FAKRO*/
	
	$query = "SELECT
				Sum(zp_ilosc*zp_cena) AS wartosc_netto ,
				Sum(round(zp_ilosc*zp_cena*(100+to_vat))/100) AS wartosc_brutto
				FROM
				zampoz,towar_sklep,towar
				WHERE
				ts_id=zp_ts_id AND
				to_id=ts_to_id AND
				zp_za_id = $za_id";
	...
	$query = "SELECT
				Sum(zp_ilosc_ws*zp_cena_ws) AS wartosc_netto_ws ,
	...
	/*FAKRO*/
	#$_wartosc_brutto = ($za_poczta_br+$wartosc_netto+$tr_strefa_vat);
	#$wartosc_brutto_zl = u_cena($_wartosc_brutto).' | '.$za_poczta_br.'+'.$wartosc_netto.'+'.$tr_strefa_vat;
	$wartosc_brutto_zl = u_cena($za_poczta_br+$wartosc_netto+(($wartosc_netto*$tr_strefa_vat)/100));
	/*FAKRO*/
