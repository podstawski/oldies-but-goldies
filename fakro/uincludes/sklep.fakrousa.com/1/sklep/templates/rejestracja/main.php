...
/*FAKRO*/
	...
	$tr_lista_1 = "<option value=\"\">--".sysmsg('Select','user')."-----</option>";
	$tr_lista_2 = "<option value=\"\">--".sysmsg('Select','user')."-----</option>";
	for($i=0; $i < $res->RecordCount(); $i++) {
		parse_str(ado_explodename($res,$i));
		unset($tr_strefa_name_selected);
		
		$tr_lista_1 .= "<option value=\"$tr_strefa_name\" ".(($su_stan == $tr_strefa_name)?'selected':'').">$tr_strefa_opis</option>";
		$tr_lista_2 .= "<option value=\"$tr_strefa_name\" ".(($adr1_stan == $tr_strefa_name)?'selected':'').">$tr_strefa_opis</option>";
		}
/*FAKRO*/
	...
	$osoby_fields_stan = sysmsg("State","user");
