...
$sql = "SELECT tr_typ.tr_typ_name, tr_strefa.tr_strefa_opis FROM tr_strefa,tr_typ WHERE tr_strefa.tr_strefa_typ = tr_typ.tr_typ_id AND tr_strefa.tr_strefa_id = $za_poczta";
parse_str(ado_query2url($sql));
...
if(!strlen($za_adres)) {
	...
	$za_adres = str_replace(" "," ","$za_adres, $su_stan");
	#$za_adres = $su_adres1; /*fakro*/
	}

if (!strlen($su_nazwa)) $su_nazwa = "\n$su_imiona $su_nazwisko\n$su_ulica\n$su_kod_pocztowy $su_miasto, $su_stan";

#$su_nazwa = "\n$su_nazwa_firmy";

if($za_data_przyjecia) $za_numer.= " / ".date('d-m-Y H:i',$za_data_przyjecia);
...
/**/
$pdf->SetFont('verdana','',12);
$pdf->Cell(30,6,_convert(sysmsg("Telephone","user").": "),0,0,"L",0);
$pdf->SetFont('verdanab','',12);
$pdf->Cell(75,6,_convert($su_telefon),0,0,"L",0);
$pdf->Ln();
/**/
$pdf->Ln();
$pdf->SetFont('verdana','',12);
...
$pdf->SetFont('verdanab','',12);
$pdf->Cell(35,6,_convert($tr_typ_name),0,0,"L",0);
$pdf->Ln();
$pdf->SetFont('verdana','',12);
$pdf->Cell(30,6,_convert(sysmsg("Client order","cart").": "),0,0,"L",0);
$pdf->SetFont('verdanab','',12);
$pdf->Cell(45,6,_convert($za_numer_obcy),0,0,"L",0);
...
$pdf->Cell(55,5,_convert(sysmsg("Article name","cart")),1,0,"L",1);
$pdf->Cell(25,5,_convert(sysmsg("Article index","cart")),1,0,"L",1);
$pdf->Cell(20,5,_convert(sysmsg("Article sizes","cart")),1,0,"L",1);
$pdf->Cell(15,5,_convert(sysmsg("Quantity","cart")),1,0,"L",1);
...
*/
	$pdf->Cell(55,5,_convert($to_indeks),1,0,"L",0);
	$pdf->Cell(25,5,_convert($to_ean),1,0,"L",0);
	...
	$pdf->Cell(15,5,_convert($zp_ilosc),1,0,"R",0);
...
/*FAKRO*/
$pdf->Cell(147,5,_convert(sysmsg("Tax","cart")),1,0,"R",0);
if($AUTH[p_price]) {
	$pdf->Cell(25,5,_convert(u_cena(($za_wart_br*$tr_strefa_vat)/100)),1,0,"R",0);
	}
$pdf->Ln();
$pdf->Cell(147,5,_convert(sysmsg("Delivery cost","cart")),1,0,"R",0);
if($AUTH[p_price]) {
...
$pdf->Ln();
$pdf->Cell(115,6,"",0,0,"L",0);
$pdf->Cell(20,6,_convert(sysmsg("Total value gross","cart").":"),0,0,"L",0);
$pdf->SetFont('verdanab','',12);
$pdf->Cell(30,6,_convert(u_cena($za_poczta_br+$za_wart_br+(($za_wart_br*$tr_strefa_vat)/100))),0,0,"L",0);
$pdf->Ln();
$pdf->Ln();
?>
