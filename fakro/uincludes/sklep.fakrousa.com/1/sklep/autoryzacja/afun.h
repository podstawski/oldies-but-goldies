...
if (!function_exists("system_user_additional"))
{
	...
	{
		//if ($obj[projekt]) return $obj; // ustawiam tylko raz na sesjê
		...
		$query="SELECT su_telefon, su_nazwisko, su_login, su_dostawa, su_platnosc, su_blokady,
			su_ulica,su_kod_pocztowy,su_miasto, su_nip, su_stan FROM system_user WHERE su_id=$obj[parent]";
		parse_str(ado_query2url($query));
		...
		$obj[adres] = $su_ulica.", ".$su_miasto." ".$su_stan." ".$su_kod_pocztowy;
		$obj[nip] = $su_nip;
		return $obj;
	}
}
