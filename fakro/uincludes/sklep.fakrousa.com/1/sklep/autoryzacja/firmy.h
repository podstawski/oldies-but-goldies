...
<tr>
	<?
		echo "<th sort=\"$of[1]\">".sysmsg("No","system")."</th>\n";
	...
	echo "<th class=\"co\">".sysmsg("Actions","system");
	...
	for ($i=0;$i<$result->RecordCount();$i++)
	{
		...
		if ($nextpage!=$self) $buttons = "<a href=\"$nextpage${next_char}sc[kontrahent_id]=$su_id&".sort_navi_qs($LIST)."\"><img src=\"$SKLEP_IMAGES/i_lista.gif\" alt=\"".sysmsg('Order','system')."\" border=0></a>&nbsp;";

		if ($su&2) $buttons.="<img src=\"$SKLEP_IMAGES/i_delete.gif\" style=\"cursor:hand\" onClick=\"list_delete_item('FirmaUsun','$su_id:$sg:$sg_coid')\"  alt=\"".sysmsg('Delete','system')."\">";
