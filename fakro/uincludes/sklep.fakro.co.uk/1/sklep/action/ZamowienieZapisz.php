<?
	...
	/*FAKRO*/
	$sql_user = "SELECT * FROM system_user WHERE su_id = ".$AUTH[id];
	parse_str(ado_query2url($sql_user));
	
	$FORM[user_address]		= $su_ulica.', '.$su_miasto;
	$FORM[user_postcode]	= $su_kod_pocztowy;
	$FORM[user_name]		= $su_imiona.' '.$su_nazwisko;
	$FORM[user_tel]			= $su_telefon;
	$FORM[user_email]		= $su_email;
	/*FAKRO*/
...
//  sprawdzenie czy user ma voucher
  ...
  if($ko_rez_uwagi) {
    ...
    if($ri_procent) {
      $suma_br_voucher = $ri_procent;
      $suma_nt_voucher = $ri_procent;
      
      $query.=";
      UPDATE zamowienia SET za_voucher_id='$ko_rez_uwagi' WHERE za_id=$zamowienie_id;";

      $suma_br = ($suma_br-(($suma_br*$suma_br_voucher)/100));
      $suma_nt = ($suma_nt-(($suma_nt*$suma_nt_voucher)/100));
