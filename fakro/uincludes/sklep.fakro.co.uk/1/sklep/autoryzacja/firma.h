<?
	...
	$sj_tr="";
	//$sj_tr.="<tr><th colspan=2>Assigned to shops</th></tr>";
...
<tr><td colspan=2>
	<?if ($su&1) { ?>
	<input type="button" class="sys_button" value="Back" onClick="document.rollbackForm_<? echo $sid ?>.submit()">
	<? }  ?>
	<input type="submit" class="sys_button" value="Save">
	</td></tr>
</tfoot>
</table>
</form>
...
function validateForm(obj)
{
	...
	for (i=0;i<oColLen;i++ )
	{
		...
		if (wyr==1)
		{
			...
			{
				if (oElem.value.length==0) alert('Data not provided in space "'+oElem.title+'"'); 
				else alert('Za ma³o znaków w polu "'+oElem.title+'"');
				oElem.focus();
				return false;
			}
