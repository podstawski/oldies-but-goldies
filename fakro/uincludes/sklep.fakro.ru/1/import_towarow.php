<?
       ...
       if (is_array($files_to_import))
       {
              ...
              while (list(,$filename) = each($files_to_import))
              {
                     ...
                     while (list($arkusz,$cell) = each($xml))
                     {
                            ...
                            for ($i=0;$i<count($cell[0]);$i++)
                            {
                                   ...
                                   {
                                          echo sysmsg("Brak zrozumienia dla nagГѓwka","import")." <b>$header</b>, ".sysmsg("pomijam kolumnъ","import")."<sup>1</sup><br>";
                                          $hints[1]=sysmsg("Dostъpne pola w nagГѓwku","import").': '.implode(', ',array_keys($slownik_naglowka));
                            ...
                            for ($i=1; $i < count($cell); $i++)
                            {
                                   ...
                                   if (!$ts_id)
                                   {
                                          $sql="INSERT INTO towar_sklep (ts_to_id,ts_sk_id) VALUES ($to_id,$SKLEP_ID); $query";
                                          parse_str(ado_query2url($sql));
                                   }
              ...
              echo "<br>&nbsp; Dodano $ile_towarow_dodano, uaktualniono $ile_towarow towarѓw<br>";
       }
