		...
		while (list(,$filename) = each($files_to_import))
		{
			...
			while (list(,$cell) = each($xml))
			{
				for ($i=0; $i < count($cell); $i++)
				{
					...
					{
						$rozmiary=array();
						for ($r=1; $r < count($row); $r++) $rozmiary["".$row[$r].""]=$r;
...
//							echo $nazwa_produktu."<br>";continue;
		...
		echo "Uaktualniono ".$ile_towarow." towarѓw<br>";
	}
