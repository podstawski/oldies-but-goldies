...
public class subiekt24 : WebService {
	...
	tmpK = zamowienie.kontrahent;
        ...
        try
        {
            //jeХМeli go nie ma to go dodajemy
            if (!Any)
            {
				...
				catch
                {
					...
					if (ile > 0)
					{
						errString = "BХФd przy pobieraniu identyfikatora kontrahenta.";
                ...
                {
                    errString = "BХФd przy dodawaniu kontrahenta";
            ...
            if (!Any)
            {
                ...
                if (!reader.Read())
                {
					...
					if (ile > 0)
					{
						errString = "BХФd przy pobieraniu identyfikatora ewid";
						Exception noId = new Exception(errString);
						throw (noId);
					}
					else
...
//				return null;
                ...
                if (command.ExecuteNonQuery()==0)
                {
                    errString = "BХФd przy dodawaniu ewid";
                    Exception noInsert = new Exception(errString);
                    throw (noInsert);
				...
				catch
				{
					...
					if (ile >0)
					{
						errString = "BХФd przy pobieraniu identyfikatora hist";
						Exception noId = new Exception(errString);
						throw (noId);
					}
					else
                ...
                if (command.ExecuteNonQuery()==0)
                {
                    errString = "BХФd przy dodawaniu historii";
                    Exception noInsert = new Exception(errString);
                    throw (noInsert);
            ...
            if (adrh_id == -1)
            {
				...
				{                    
					...
					if (ile > 0)
					{
						errString = "BХФd przy pobieraniu nr historii";
						Exception noId = new Exception(errString);
						throw (noId);
					}
					else
			...
			catch
			{
				...
				if (ile > 0)
				{
					errString = "BХФd przy pobieraniu nr dok";
					Exception noId = new Exception(errString);
					throw (noId);
				}
				else
	       ...
	       "\'"+zamowienie.kontrahent.ko_kod+"\')";
            ...
            if (command.ExecuteNonQuery()==0)
            {
                errString = "BХФd przy dodawaniu zamУГwienia";
                Exception noInsert = new Exception(errString);
                throw (noInsert);
			...
			catch
			{
				...
				if (ile > 0)
				{
					errString = "BХФd przy pobieraniu nr pozycji";
					Exception noId = new Exception(errString);
					throw (noId);
				}
				else
...
*/
                ...
                if (command.ExecuteNonQuery()==0)
                {
                    errString = "BХФd przy dodawaniu pozycj";
                    Exception noInsert = new Exception(errString);
                    throw (noInsert);
