<?
	...
	if (!is_object($res))
	{
		...
		if ($projdb->Execute($query))
				echo "Wykonano inicjacjъ<br>";
		else
		{
			echo "Nie udaГo siъ dokonaц inicjacji<br>";
			if ($KAMELEON_MODE)
			{
				$projdb->debug=1;
				echo "<pre>";
	...
	if ($dh = opendir($dir)) 
	{
		while (($file = readdir($dh)) !== false)
		{
			...
			if ($projdb->Execute($query))
				echo "Wykonano zmianъ w bazie danych na podstawie pliku $file<br>";
			else
			{
				echo "Nie udaГo siъ dokonaц zmian na podstawie pliku $file<br>";
				if ($KAMELEON_MODE)
				{
					$projdb->debug=1;
					echo "<pre>";
