<?
	...
	$sj_tr="";
	//$sj_tr.="<tr><th colspan=2>Przypisana do sklepѓw</th></tr>";
...
<tr><td colspan=2>
	<?if ($su&1) { ?>
	<input type="button" class="sys_button" value="<? echo sysmsg("Return","admin"); ?>" onClick="document.rollbackForm_<? echo $sid ?>.submit()">
	<? }  ?>
	<input type="submit" class="sys_button" value="<? echo sysmsg("Save","admin"); ?>">
	</td></tr>
</tfoot>
</table>
</form>
	...
	for (i=0;i<oColLen;i++ )
	{
		...
		if (wyr==1)
		{
			if (oElem.minlength != null && oElem.minlength>oElem.value.length )
			{
				...
				else alert('Za maГo znakѓw w polu "'+oElem.title+'"');
				oElem.focus();
				return false;
			}
		...
		if (wyr=='email')
		{
			...
			{
				alert('NieprawidГowy format adresu E-mail');
				oElem.focus();
				return false;
			}
		}
