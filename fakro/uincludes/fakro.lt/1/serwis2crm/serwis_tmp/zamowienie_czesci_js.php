...
function validate(address, daneJakWyzej)
{
	...
	if(!el('data_montazu').disabled)
	{
		...
		if(el('data_montazu').value < el('data_nabycia_towaru').value)
		{
			return inform_date_not_less('data_nabycia_towaru',
				'Data monta¿u nie mo¿e byæ wcze¶niejsza ni¿ data nabycia towaru!');
		}
	}
    ...
    {
        alert('Musisz podaæ numer telefonu!');
        el('t1_numer').focus();
	    return false;
	}
	...
	{
		alert('Aby kontynuowaæ nale¿y przeczytaæ i zaakceptowaæ regulamin!');
		return false;
	}
...
<? for($i = 0; $i < count($_czesci); $i++) { ?>
	...
	{
		alert('Musisz podaæ poprawn± ilo¶æ zamawianej czêsci!');
		count.focus();
		return false;
	}
<? } ?>
...
function display(id)
{
    ...
    explozyjny = id;
	window.scrollTo(0,0);
	// el('info').innerHTML += '<a href="#" onClick="add(\'' + id + '\');"> Dodaj czê¶æ do zamówienia</a>';
}
