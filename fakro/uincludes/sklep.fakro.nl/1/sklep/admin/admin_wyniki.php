...
//	$qs = ereg_replace("\+ ","\+",$qs);
	...
	if (!is_object($result)) 
	{
		echo "U¿yto nieprawid³owych znaków przy wyszukiwaniu !!!<br>"; 
		return;
	...
	for ($i=0; $i < $result->RecordCount(); $i++)
	{
		...
		$options="
		<A HREF=\"$more${next_char}list[id]=$to_id&".sort_navi_qs($LIST)."\"><img src=\"$UIMAGES/autoryzacja/i_editmode_n.gif\" hspace=5 vspace=0 border=0 style=\"cursor:hand\" alt=\"edycja ogólnych parametrów towaru - nazwy, wymiarów, itp\"></A>
		<A HREF=\"$next${next_char}list[id]=$to_id&".sort_navi_qs($LIST)."\"><img src=\"$UIMAGES/autoryzacja/i_tree_n.gif\" hspace=5 vspace=0 border=0 style=\"cursor:hand\" alt=\"zmiana cen i innych parametrów zwi±zanych z handlem\"></A>
		";
