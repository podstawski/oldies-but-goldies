...
function search()
{
	...
	$smarty->assign('title', 'Zamѓwienie czъЖci');
...
function results()
{
    ...
    $smarty->assign('title', 'Zamѓwienie czeЖci');
	$smarty->assign('zglaszajacy_caption','Dane do faktury');
	$smarty->assign('adres_zamontowania_caption','Adres dostarczenia przesyГki');
	...
	$content .= '<a href="?module=bp&mode=search">Powrѓt</a>';
...
function details()
{
    ...
    foreach ($_POST['ilosc'] as $id_bp_czesci => $ilosc)
	{
		if ($ilosc > 0)
		{
			...
			$dodatkowyKomentarz .= ' '.$_POST['czesci'][$id_bp_czesci] . ' (IloЖц: '.$ilosc.', ' .
