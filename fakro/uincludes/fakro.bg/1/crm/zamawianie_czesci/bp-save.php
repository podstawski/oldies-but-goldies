...
function summary()
{
	...
	$row['title'] = 'Zamѓwienie czъЖci - podsumowanie';
	$row['zglaszajacy_caption'] = 'Dane zamawiajБcego';
	return $row;
}
...
function save()
{
	...
	unset($_SESSION['done']);
	$_SESSION['done']['produkty'] = 'Produkty, dla ktѓrych zamѓwiono czeЖci:';
	...
	if($_POST['sposob_platnosci'] == 2) $sposob_platnosci = 'PГatnoЖci elektroniczne (karta pГatnicza, szybki przelew, przelew bankowy)';
	...
	$zgloszenie_serwisowe = array(
		...
		'uwagi' => 'Wybrany dostawca: ' . $_POST['nazwa_dostawcy'] . '.--BR--CaГkowity koszt zamѓwienia: ' . $_POST['suma'] . ' PLN.--BR--'.$sposob_platnosci.'--BR--',
	...
	foreach($_POST['produkty'] as $tabliczka => $array)
	{
	    ...
	    foreach ($array['ilosc'] as $id_bp_czesci => $ilosc)
		{
			if ($ilosc > 0)
			{
				...
				$dodatkowyKomentarz .= '--BR--'.$array['czesci'][$id_bp_czesci] . ' (IloЖц: '.$ilosc.', ' .
		...
		{
			$dodatkowyKomentarz = ' Zamѓwione czъЖci:--BR--' . $dodatkowyKomentarz . '--BR--';
