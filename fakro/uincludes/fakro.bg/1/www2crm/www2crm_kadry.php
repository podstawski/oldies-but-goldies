...
if(PEAR::isError($dba)) {
	...
	$re = $dba->query("INSERT INTO www2crm_kadry_kandydat (
					...
					strtoupper($podstawowe['tel_kom_op']),strtoupper($podstawowe['tel_kom']),strtoupper($podstawowe['wojewodztwo']),$inne['typ']
					)
					);
	...
	if(DB::isError($re)) {
		...
		$komunikat = 'Dziъkujemy za wypeГnienie formularza.';
		}
	}
