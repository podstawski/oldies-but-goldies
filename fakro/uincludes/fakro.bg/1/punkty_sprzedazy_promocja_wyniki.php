...
//	$fakrodb->debug=1;
	$sql = "SELECT * FROM punkty_sprzedazy WHERE ps_typ='P' AND ps_ma_okno='1' $cond ORDER BY ps_wojewodztwo, ps_miasto, ps_nazwa";
	$res = $fakrodb->execute($sql);

	if (!$LIST[ile])
	{
		$query="SELECT count(ps_id) AS c FROM punkty_sprzedazy WHERE ps_typ='P' AND ps_ma_okno='1' $cond";
