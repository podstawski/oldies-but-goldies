...
<TR>
	<TD valign="top">Konwersja:</TD>
	<TD><? echo $conv ?></TD>
</TR>

<TR>
	<TD valign="middle">Linie:</TD>
	<TD>
		od <input type="text" size=4 name="LINIA[od]" value="<?echo $LINIA[od]?>">
		do <input type="text" size=4 name="LINIA[do]" value="<?echo $LINIA["do"]?>">
		co <input type="text" size=4 name="LINIA[step]" value="<?echo $LINIA["step"]?>">
		(Uwaga, nie wszystkie moduły obsługują ograniczenie) 
	</TD>
</TR>
<TR>
...
<?
	...
	if (is_array($pliki) && count($pliki)>1)
	{
		...
		echo "<input type='checkbox' $checked name='import_pliki[IMPLODE]' value=':'> 
				<font color=red><i>wszystkie zaznaczone w jednej paczce</i></font><br>";
	}
	...
	{
		$LINIA["step"]=0;
		echo "Brak ograniczenia górnego - nie można wykonać krokowo.";
	}
...
?>
<TR>
	<TD valign="top" colspan=2><INPUT TYPE="submit" value="Importuj"></TD>
</tr>
</TABLE>
</FORM>
