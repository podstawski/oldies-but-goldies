<?
	...
	if (!is_object($res))
	{
		...
		if ($projdb->Execute($query))
				echo "Wykonano inicjację<br>";
		else
		{
			echo "Nie udało się dokonać inicjacji<br>";
			if ($KAMELEON_MODE)
			{
				$projdb->debug=1;
				echo "<pre>";
	...
	if ($dh = opendir($dir)) 
	{
		while (($file = readdir($dh)) !== false)
		{
			...
			if ($projdb->Execute($query))
				echo "Wykonano zmianę w bazie danych na podstawie pliku $file<br>";
			else
			{
				echo "Nie udało się dokonać zmian na podstawie pliku $file<br>";
				if ($KAMELEON_MODE)
				{
					$projdb->debug=1;
					echo "<pre>";
