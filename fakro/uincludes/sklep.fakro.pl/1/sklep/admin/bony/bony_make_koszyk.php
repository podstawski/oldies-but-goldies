<?
//  if($_SERVER['REMOTE_ADDR'] != '89.174.30.19') return;
?>
<?
  ...
  if($AUTH['id']>0) {
    ...
    if($user_action['action'] == 'add_voucher') {
      $sql1 = "SELECT * FROM voucher WHERE su_id=".$AUTH['id']." AND voucher_type_id='".$voucher_type_id."' AND voucher_status=2 AND voucher_name='".$user_action['voucher_name']."'";
      ...
      for($i1=0; $i1 < $res2->RecordCount(); $i1++) {
        ...
        $query = "UPDATE koszyk SET ko_opcje='".$ko_opcje_su_id."', ko_rez_uwagi='".$voucher_id."' WHERE ko_id='".$ko_id."' ;";
