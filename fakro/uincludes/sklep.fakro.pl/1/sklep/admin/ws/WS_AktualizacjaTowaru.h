...
<form method="post" action="<?echo $self?>">
	...
	<input type="button" value="Synchronizuj towary z programem magazynowym" class="addbut"
...
<?
	$query="SELECT to_id FROM towar ORDER BY to_id";
...
?>
	...
	{	
		...
		html='Postęp wykonania: pozycja '+j+' z '+TOWAR_COUNT;	
		span=getObject('syncSpan');		
		if (i==0 && span.innerHTML.length>0)
		{
			alert('Działa już jedna synchronizacja');
			return;
		}
		...
		{
			alert('Działa tylko poza kameleonem');
			return;
		}

		r=Math.random();
		href='<? echo "$self$next_char"?>a='+r+'&action=WS_AktualizacjaTowaru&list[to_id]='+TOWARY[i]+'&js_action=synchronizuj('+j+')';
