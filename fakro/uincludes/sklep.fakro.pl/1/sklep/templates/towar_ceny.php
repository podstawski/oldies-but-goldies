...
//  $total_value_br+=$wart_br;
  ...
  $total_value_br=($total_value_br_tmp-$voucher_wartosc);
  $total_value_br_zl=u_Cena($total_value_br_tmp-$voucher_wartosc);
  
  if($total_value_br<0) $total_value_br=0;
  if($total_value_br_zl<0) $total_value_br_zl=u_Cena(0);
  
  // podliczenie bonu
  if($total_value_br_zl<1) {
    $voucher_error['998'] = 1;
    $koszyk_button_checkout_akceptacja = '<INPUT TYPE="button" onClick="document.prepareResForm.submit()" value="'.sysmsg('Checkout Voucher','cart').'" class="button_o">';
  }else{
    $voucher_error['998'] = 0;
    $koszyk_button_checkout = ($total_value_br_zl<1)?'':'<INPUT TYPE="button" onClick="document.prepareResForm.submit()" value="'.sysmsg('Checkout','cart').'" class="button_o">';
  }
  //
