...
//    alert("Hasło nie zgodne z potwierdzeniem. Prosze poprawić.");
    obj1.focus();
    return true;
  }
  return false;
...
function czyStareHasloOK(obj1,obj2)
{
  ...
  if (obj1.value!=obj2.value) {
    alert("Proszę wpisać poprawnie dotychczasowe hasło.");
    obj1.focus();
    return true;
  }
  return false;
...
if (min!=-1) {
  if (obj.value.length < min) {
    obj.focus();
    //alert("Za mało znaków, proszę wypełnić pole. " +obj.name);
    return  true;
  }
  if (max!=-1) 
    if (obj.value.length > max) {
      obj.focus();
      //alert("Za dużo znaków w polu: "+obj.name);
      return  true;
    }
}
else
...
if (min!=-1) {
  if (obj.value.length < min) {
    alert("Za mało znaków, proszę poprawnie wypełnić pole. ");
    obj.focus();
    return  true;
  }
  if (max!=-1) 
    if (obj.value.length > max) {
      alert("Za dużo znaków, proszę poprawnie wypełnić pole ");
      obj.focus();
      return  true;
    }
}
...
var text, znak, ERR;
  ...
  if (ERR) {
   alert("W tym polu należy wpisac liczbę całkowitš");
   obj.focus();
   return ERR;
  }
if (min!=-1) {
...
var text, znak, ERR;
  ...
  if (isNaN(liczba)) ERR=1;
  if (ERR) {
   alert("W tym polu należy wpisac liczbę zmiennoprzecinkowš z kropkš.");
   obj.focus();
   return ERR;
  }
if (min!=-1) {
