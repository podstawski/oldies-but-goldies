...
class WM extends DB
{
	...
	function produkty_zamowienia($za_id, $za_notpola='')
	{
		...
		$ret.="<table cellpadding=\"0\" cellspacing=\"0\" class=\"zampoz\">\n";
		$ret.="<thead><tr>";
		if (!strstr($za_notpola,'lp')) $ret.="<td class=\"lp\">".sysmsg("Lp.","cart")."</td>";
		if (!strstr($za_notpola,'an')) $ret.="<td class=\"an\">".sysmsg("Indeks","cart")."<br>".sysmsg("Article name","cart")."</td>";
		...
		if (!strstr($za_notpola,'qu')) $ret.="<td class=\"qu\">".sysmsg("Gross price","cart")."</td>";
		...
		for ($i=0; $i < $res->RecordCount(); $i++)
		{
			...
			if (!strstr($za_notpola,'an')) $ret.="<td class=\"an\">".$to_indeks.$to_ean."<br>".stripslashes(sysmsg($to_nazwa,'towar'))." </td>";
...
if($za_voucher_id) {
  $sql1 = "SELECT * FROM voucher WHERE voucher_id=".$za_voucher_id;
  ...
  $voucher_table = 'Voucher '.$voucher_wartosc.' PLN';

  $ret.="<tr class=\"kp\">";
  if (!strstr($za_notpola,'lp')) $ret.="<td class=\"lp\">".($i+1)." </td>";
  if (!strstr($za_notpola,'an')) $ret.="<td class=\"an\">".stripslashes($voucher_table)." </td>";
  if (!strstr($za_notpola,'qu')) $ret.="<td class=\"qu\"> </td>";
  if (!strstr($za_notpola,'pn')) $ret.="<td class=\"pn\">".u_cena($voucher_wartosc_table)." </td>";
  if (!strstr($za_notpola,'pb')) $ret.="<td class=\"pb\">".u_cena($voucher_wartosc_table)." </td>";
  if (!strstr($za_notpola,'vn')) $ret.="<td class=\"vn\">".u_cena($voucher_wartosc_table)." </td>";
  if (!strstr($za_notpola,'vb')) $ret.="<td class=\"vb\">".u_cena($voucher_wartosc_table)." </td>";
  $ret.="</tr>";
  
  $total_value_n = ($total_value_n-$voucher_wartosc);
  $total_value_b = ($total_value_b-$voucher_wartosc);
    
  if($total_value_n<0) $total_value_n = 0;
  if($total_value_b<0) $total_value_b = 0;  
}else{
  $voucher_lp = 1;
  $voucher_wartosc = 0;
}
//
		...
		{
			$ret.="<tr class=\"kp\">";
			if (!strstr($za_notpola,'lp')) $ret.="<td class=\"lp\">".($voucher_lp+1)." </td>";
			if (!strstr($za_notpola,'an')) $ret.="<td class=\"an\">".stripslashes($po_nazwa)." </td>";
			if (!strstr($za_notpola,'qu')) $ret.="<td class=\"qu\">1 </td>";
			if (!strstr($za_notpola,'pn')) $ret.="<td class=\"pn\">".u_cena($za_poczta_nt)." </td>";
			if (!strstr($za_notpola,'pb')) $ret.="<td class=\"pb\">".u_cena($za_poczta_br)." </td>";
			if (!strstr($za_notpola,'vn')) $ret.="<td class=\"vn\">".u_cena($za_poczta_nt)." </td>";
