...
/* Duplikuje się 2006-05-15 Camel
	if ($show_index)
	{
...
*/
    ...
    //$total_value+= ($zp_cena*$zp_ilosc);
		$total_value+= ($zp_cena*$zp_ilosc*((100+$to_vat)/100));
	}
...
if($za_voucher_id) {
  ...
  $pdf->Cell(7,5,_convert(($i+1)),1,0,"L",0);
  $pdf->Cell(50,5,_convert($voucher_table),1,0,"L",0);
  ...
  if ($AUTH[p_price])
  {
    $pdf->Cell(25,5,_convert(u_cena($voucher_wartosc_table)),1,0,"R",0);
    $pdf->Cell(25,5,_convert(u_cena($voucher_wartosc_table)),1,0,"R",0);
  }

  $pdf->Ln();
  
  $za_wart_br = ($total_value-$voucher_wartosc);
...
//  
	...
	$pdf->Cell(7,5,_convert(($i+$voucher_lp)),1,0,"L",0);
	$pdf->Cell(50,5,_convert($po_nazwa),1,0,"L",0);
	$pdf->Cell(35,5,_convert(""),1,0,"L",0);
...
//	$pdf->SetFont('verdanabpl','',12);	
//	$pdf->Cell(30,6,_convert(u_cena($za_wart_nt-$voucher_wartosc)),0,0,"L",0);
//	$pdf->Ln();
	$pdf->Cell(115,6,"",0,0,"L",0);
	$pdf->SetFont('verdanapl','',12);
