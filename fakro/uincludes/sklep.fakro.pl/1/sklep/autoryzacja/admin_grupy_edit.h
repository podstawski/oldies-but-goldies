<?
	...
	if (strlen($sg_id))
	{
		...
		if ($priv[p_admin]) $chck1 = "checked";
		if ($priv[p_price]) $chck2 = "checked";
		if ($priv[p_order]) $chck3 = "checked";
	}
	...
	<TR>
		<TD colspan=2>
			nazwa grupy <INPUT TYPE=\"text\" NAME=\"nazwa_grupy\" value=\"$sg_nazwa\" style=\"forminput\">
		</TD>
		<TD>
		</TD>
		<TD rowspan=2 valign=\"top\">
			<INPUT TYPE=\"button\" value=\"Anuluj\" class=\"sys_button\" onClick=\"document.goBackForm.submit()\">
	...
	<tr>
		...
		<td >
			<INPUT TYPE=\"checkbox\" NAME=\"PRIV[is_admin]\" $chck1 value=\"1\"> Administrator<br>
			<INPUT TYPE=\"checkbox\" NAME=\"PRIV[see_prices]\" $chck2 value=\"2\"> Widzi ceny<br>
			<INPUT TYPE=\"checkbox\" NAME=\"PRIV[do_orders]\" $chck3 value=\"4\"> Zamówienia<br>
		</td>
	</tr>
	<TR>
...
<script>
	...
	function changeRights(id,val)
	{
		...
		ile = c_len+1;
		if (c_len)
			if (confirm('Na pewno ustawić/zabrać wszystkie ('+ile+') prawa ?'))
