<?
	...
	$slownik_naglowka = array (
		...
		'ean' => 'towar->to_ean',
		'vat' => 'towar->to_vat',
		'zdjęcie' => 'towar->to_foto_s',
		'cena' => 'towar_sklep->ts_cena',
		'kategoria' => 'kategoria[]',
		'szerokość' => 'towar_parametry->tp_a',
		'wysokość' => 'towar_parametry->tp_b',
		...
		while (list(,$filename) = each($files_to_import))
		{
			...
			while (list($arkusz,$cell) = each($xml))
			{
				...
				for ($i=0;$i<count($cell[0]);$i++)
				{
					...
					{
						echo sysmsg("Brak zrozumienia dla nagłówka","import")." <b>$header</b>, ".sysmsg("pomijam kolumnę","import")."<sup>1</sup><br>";
						$hints[1]=sysmsg("Dostępne pola w nagłówku","import").': '.implode(', ',array_keys($slownik_naglowka));
