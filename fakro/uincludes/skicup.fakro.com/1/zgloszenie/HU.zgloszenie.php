...
<tr>
	...
	<dd>Elnézését kérjük, de az IFD FAKRO SÍVILÁGBAJNOKSÁG jelentkezési listája lezárásra került
	Köszönjük érdeklődését!</dd></div>
	<br> <div align="right">A szervezők</div></td>
</tr>
</table>
<?
	}else{
...
<br>
<? if($error_nip == 1) 	echo '<div align="center"><strong><font style="color:#ff0000;">Elnézését kérjük, de az Ön Cége túllépte a jelentkezési létszámot. Lásd: szabályzat I. 4,5 pontjai</font></div></strong><br>'; ?>
<? if($errors!='' && $error_nip != 1) echo '<div align="center"><strong><font style="color:#ff0000;">Kérem, töltse ki a pirossal jelölt mezőket!</font></div></strong><br>';?>
<br>
...
<tr>
	<td align="right"><font style="color: #<?=$imief;?>">Keresztnév</font></td>
	<td colspan="2"><input type="Text" name="form[imie]" size="30" maxlength="70" value="<? echo $form['imie'];?>" class="input_pole"></td>
</tr>
<tr>
	<td align="right"><font style="color: #<?=$nazwiskf;?>">Vezetéknév</font></td>
...
<tr>
	<td align="right"><font style="color: #<?=$emailf;?>">E-mail cím</font></td>
	<td colspan="2"><input type="Text" name="form[email]" size="50" maxlength="70" value="<? echo $form['email'];?>" class="input_pole"></td>
</tr>
<tr>
	<td align="right"><font style="color: #<?=$nipf;?>">Adóazonosító-szám (csak EU-tag esetén)</font></td>
	<td colspan="2"><input type="Text" name="form[nip]" size="50" maxlength="70" value="<? echo $form['nip'];?>" class="input_pole"></td>
</tr>
<tr>
	<td align="right"><font style="color: #<?=$osoba_towarzyszacaf;?>">Kísérőszemély</font></td>
	<td colspan="2">
	<input type="radio" name="form[osoba_towarzyszaca]" value="tak" <? if($form['osoba_towarzyszaca'] == "tak") {?>checked<?}?>> Igen
	<br>
	<input type="radio" name="form[osoba_towarzyszaca]" value="nie" <? if($form['osoba_towarzyszaca'] == "nie") {?>checked<?}?>> Nem
	</td>
</tr>
<tr>
	<td align="right">Keresztnév</td>
...
<tr>
	<td align="right"><font style="color: #<?=$hotelf;?>">Szállásfoglalás (Hotel Activa):</font></td>
	<td colspan="2">
	<input type="radio" name="form[hotel]" value="POKOJ 1-OSOBOWY" <? if($form['hotel'] == "POKOJ 1-OSOBOWY") {?>checked<?}?>> EGYÁGYAS SZOBA
	<br>
	<input type="radio" name="form[hotel]" value="POKOJ 2-OSOBOWY" <? if($form['hotel'] == "POKOJ 2-OSOBOWY") {?>checked<?}?>> KÉTÁGYAS SZOBA
	</td>
</tr>
<tr>
	<td align="right"><font style="color: #<?=$wypozyczenie_sprzetuf;?>">Sífelszerelést a helyszínen bérelnék</font></td>
	<td colspan="2">
	<input type="radio" name="form[wypozyczenie_sprzetu]" value="tak" <? if($form['wypozyczenie_sprzetu'] == "tak") {?>checked<?}?>> Igen
	<br>
	<input type="radio" name="form[wypozyczenie_sprzetu]" value="nie" <? if($form['wypozyczenie_sprzetu'] == "nie") {?>checked<?}?>> Nem
	</td>
</tr>
<tr>
	<td colspan="3"><br></td>
...
<tr>
	<td align="right"><font style="color: #<?=$transportf;?>">Érkezés a helyszínre:</font></td>
	<td colspan="2">
	<input type="radio" name="form[transport]" value="transport zorganizowany" <? if($form['transport'] == "transport zorganizowany") {?>checked<?}?>> repülőgéppel Krakkóig ("Balice" Nemzetközi Repülőtér),
	majd a szervező által biztosított transzfer Muszynába
	<br><br>
	<input type="radio" name="form[transport]" value="transport wlasny" <? if($form['transport'] == "transport wlasny") {?>checked<?}?>> egyéni módon
	</td>
</tr>
</table>
...
<tr>
	...
	<td><div align="justify">Hozzájárulok a jelentkezési lapon közölt személyes adataim szervezők általi feldolgozására
	az IFD FAKRO SÍ VIKÁGKUPÁVAL kapcsolatosan a hatályos adatvédelmi törvényeknek megfelelően.</div></td>
</tr>
<tr>
	<td colspan="2"><br></td>
</tr>
<tr>
	...
	<td><div align="justify"><font style="color: #<?=$oswiadczenief;?>">Elfogadom az IFD FAKRO SÍ VILÁGKUPA versenyszabályzatát.</font></div></td>
</tr>
</table>
<table width="550" border="0" cellspacing="3" cellpadding="3">
<tr>
	<td width="200"></td>
	<td width="350"><input type="submit" name="poczta" value="Jelentkezés elküldése" class="submit"></td>
</tr>
</form>
</table>
...
<table width="100%" border="0" cellspacing="3" cellpadding="3">
<tr>
	<td colspan="2" align="center"><br><br><strong><font style="color:#ff0000;">- Szerverhiba -<br>- jelentkezését a rendszer nem küldte el !! -</font></strong></td>
</tr>
</table>
<br><br><br>
<?
