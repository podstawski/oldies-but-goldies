<?
	...
	if (strlen($trans_find))
	{
		$_trans_find=addslashes(utf82lang(stripslashes($trans_find),$trans_srclang));
		$_trans_replace=addslashes(utf82lang(stripslashes($trans_replace)));
		$wt_t_plain="replace(wt_t_plain,'$_trans_find','$_trans_replace')";
	}
	else
	{
	...
	$query="UPDATE webtrans SET wt_t_plain=$wt_t_plain,
