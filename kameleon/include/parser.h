...
function parser($parser_start,$parser_end,$parser_template,$parser_tokens,$parser_starter=0)
{
	...
	global $EREG_REPLACE_KAMELEON_UIMAGES,$KAMELEON_UIMAGES,$KAMELEON_UFILES,$this_editmode, $editmode, $hf_editmode;
	...
	if (strlen($parser_template) && file_exists($parser_template) )
	{
		...
		while ( in_array($parser_content[0],array(" ","\n","\r","\t")) )
		{
			$parser_pos_counter++;
			$parser_content=substr($parser_content,1);
		}
		$parser_content=trim($parser_content);
		...
		{	$lp++;
			...
			if (is_array($CONST_MORE_TOKENS) && !strlen($result_token) )
			{
				...
				foreach ($CONST_MORE_TOKENS AS $fun=>$file)
				{
			...
			{
				if (strlen(trim($result_token))) echo $result_token;
			}
			else
			switch ($parser_token)
			{
...
// header i footer
				...
				case "WEBPAGE_FOOTER" :
					...
					for ($h_i=0;$h_i<count($webtd);$h_i++)
					{
				        ...
				        if (strlen($WEBTD->costxt) && !strstr($WEBTD->costxt,'$')) 
						{	
							$colspan=0;$rowspan=0;
							parse_str($WEBTD->costxt);
							$colrowspan="";
							if ($colspan) $colrowspan.= " colspan=\"".trim($colspan)."\"";
							if ($rowspan) $colrowspan.= " rowspan=\"".trim($rowspan)."\"";
							
							if (($colspan && strpos($colspan,"rowspan")) || ($rowspan && strpos($rowspan,"colspan"))) 
								$colrowspan=" $WEBTD->costxt";
						}
...
//webtd
				...
				case "WEBTD_INCLUDE":
					...
					if (strlen($WEBTD->web20))
					{
						$web20=unserialize(base64_decode($WEBTD->web20));
						$module=$web20['module'];
						$options=$web20['options'][$module];
						if (strlen($module) && file_exists('include/web20/'.$module.'/index.php')) include('include/web20/'.$module.'/index.php');
						if (strlen($module) && $editmode && file_exists('include/web20/'.$module.'/editmode.php')) include('include/web20/'.$module.'/editmode.php');
					}
					break;
				case "WEBTD_MENU":
				...
				case "LOOP_BEGIN" :
					...
					while (1)
					{
						...
						if ( is_array($new_tokens) ) foreach($new_tokens AS $p_key=>$p_val) $parser_tokens[$p_key]=trim($p_val);
				...
				case "WEBLINK_DESCRIPTION":
					echo $WEBLINK->description; break;
                                case "WEBLINK_TITLE":
                                        echo $WEBLINK->alt_title; break;
...
//								echo "parent=$parent_page, inherit=$inheritance<br>";								
						...
						else
						{
							...
							{
							  $TD_POZIOMNAME = array();
							  for($i=0;$i<sizeof($TD_POZIOMY);$i++) $TD_POZIOMNAME[$TD_POZIOMY[$i][0]]=$TD_POZIOMY[$i][1];
								...
								if ($editmode) 
								{
								  echo "<div class=\"km_szpaltanames km_szpaltanames_body\" style=\"display: none\">".str_replace('"',"",$TD_POZIOMNAME[$level])."</div>";
								  echo "<div class=\"km_dragdrop_body km_dragdrop_place\" title=\"".$level."\" id=\"km_dragdrop".$level."\">"; // nie tyka� title = wskazuje level do kt�rego ma by� wrzucony modu�!
								  echo "<div class=\"km_draghelper\"></div>";
								  echo "<div class=\"km_dragon\" style=\"height: 1px; width: 100%;\"><a class=\"km_dragicon\"></a></div>";
								}
								if ($cr)
								{
									include("include/szpalta.h");
								}
								if ($editmode) echo "</div>";
							}
						}
					}
					elseif (ereg("WEB(HEADER|FOOTER)_LEVEL([0-9]+)",$parser_token,$levels))
					{
						...
						$TD_POZIOMNAME = array();
						for($i=0;$i<sizeof($TD_POZIOMY_HF);$i++) $TD_POZIOMNAME[$TD_POZIOMY_HF[$i][0]]=$TD_POZIOMY_HF[$i][1];
							
            
            			$cr=kameleon_td_count($page_id,$ver,$lang,$level);
						if ($editmode && $hf_editmode) 
						{
						  echo "<div class=\"km_szpaltanames km_szpaltanames".(stristr($parser_token,"WEBHEADER") ? "_head" : "_foot")."\" style=\"display: none\">".str_replace('"',"",$TD_POZIOMNAME[$level])."</div>";
						  echo "<div class=\"km_dragdrop".(stristr($parser_token,"WEBHEADER") ? "_head" : "_foot")." km_dragdrop_place\" title=\"".$level."\" id=\"km_dragdrop".$level."\">"; // nie tyka� title = wskazuje level do kt�rego ma by� wrzucony modu�!
						  echo "<div class=\"km_draghelper\"></div>";
						  echo "<div class=\"km_dragon\" style=\"height: 1px; width: 100%;\"><a class=\"km_dragicon\"></a></div>";
						}
						...
						if ($editmode && $hf_editmode) echo "</div>";

					}
					elseif (substr($parser_token,0,strlen('KAMELEON_INCLUDE_'))=='KAMELEON_INCLUDE_')
					{
						$plik='include/kameleon_'.strtolower(substr($parser_token,strlen('KAMELEON_INCLUDE_'))).'.h';

						if (file_exists($plik)) include($plik);
					}
					elseif (substr($parser_token,0,strlen('KAMELEON_'))=='KAMELEON_')
					{
						$zmienna=substr($parser_token,strlen('KAMELEON_'));
						eval( "global \$$zmienna; echo \$$zmienna;");
						
					}
					elseif (substr($parser_token,0,strlen('WEBPAGE_'))=='WEBPAGE_')
					{
						$co=strtolower(substr($parser_token,strlen('WEBPAGE_')));
		
						echo $WEBPAGE->$co;
					}
					elseif (substr($parser_token,0,strlen('WEBLINK_'))=='WEBLINK_')
					{
						$co=strtolower(substr($parser_token,strlen('WEBLINK_')));
		
						echo $WEBLINK->$co;
					}
					elseif (substr($parser_token,0,strlen('WEBTD_'))=='WEBTD_')
					{
						$co=strtolower(substr($parser_token,strlen('WEBTD_')));
		
						echo $WEBTD->$co;
					}
					elseif (substr($parser_token,0,strlen('IF:'))=='IF:')
					{
						$begin=$parser_token;
						$end='END'.$parser_token;

						$parser_content=substr($parser_content,$parser_startpos);
						$parser_if_start=$parser_startpos;
						$parser_if_end=strpos($parser_content,"%END$parser_token%");
						$parser_if_content=substr($parser_content,0,$parser_if_end);
						$parser_startpos=$parser_if_end+strlen("%END$parser_token%");

						$if=substr($parser_token,strlen('IF:'));


						$var=explode(':',$if);
						if ($pos=strpos($var[0],'->'))
						{
							$variable=substr($var[0],0,$pos);
						}
						elseif ($pos=strpos($var[0],'['))
						{
							$variable=substr($var[0],0,$pos);
						}
						else
						...
						$str2eval="global \$$variable; \$true = \$".$var[0].";";
						eval ($str2eval);
		
						if ($true)
							parser("%$parser_token%","%END$parser_token%",$parser_template,$parser_tokens);	
