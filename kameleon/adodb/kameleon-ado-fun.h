<?
// 27-10-2010 CARTMAN: zmiana nazw z ado_ na adolena_ żeby się z ado kameleona nie gryzło
 define('ADODB_FETCH_CASE', 2);
 @include_once(ADODB_DIR."adodb.inc.php");
 $ADODB_FETCH_MODE = ADODB_FETCH_ASSOC;
 function adolena_query2limit($query)
 {
	$orig_query=$query;
 ...
 function adolena_ExplodeName ($result,$row)
 {
 	$text="";
 ...
 function adolena_query2url($query)
 {
	global $lenadodb;
	echo "odwolanie";
	$new_query=adolena_query2limit($query);
	
	echo $new_query;
	return "";
  eval("\$result=\$lenadodb->SelectLimit($new_query);");
 ...
 function adolena_ObjectArray($adodb,$query)
 {
	$wynik="";
	$new_query=adolena_query2limit($query);
