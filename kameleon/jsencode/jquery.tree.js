...
(function(jQueryKam) {
	...
	jQueryKam.tree = {
		...
		defaults	: {
			...
			callback	: {
				...
				beforedata	: function(NODE,TREE_OBJ) { return { id : jQueryKam(NODE).attr("id") || 0 } }, // PARAMETERS PASSED TO SERVER
		...
		rollback	: function (data) {
			for(var i in data) {
				...
				if(data[i].selected) {
					tmp.selected = jQueryKam("#" + data[i].selected);
					tmp.selected_arr = [];
					tmp.container
		...
		drop_mode	: function (opts) {
			opts = jQueryKam.extend(opts, { show : false, type : "default", str : "Foreign node" });
			...
			if(!opts.show) {
				...
				tree_component.drag_drop.drag_help	= jQueryKam("<div id='jstree-dragged' class='tree tree-default'><ul><li class='last dragged foreign'><a href='#'><ins>&nbsp;</ins>" + opts.str + "</a></li></ul></div>");
				tree_component.drag_drop.drag_node	= tree_component.drag_drop.drag_help.find("li:eq(0)");
			}
			if(jQueryKam.tree.drag_start !== false) jQueryKam.tree.drag_start.call(null, false);
		},
		drag_start	: false,
		drag		: false,
		drag_end	: false
	};
	jQueryKam.fn.tree = function (opts) {
		return this.each(function() {
			var conf = jQueryKam.extend({},opts);
			if(tree_component.inst && tree_component.inst[jQueryKam(this).attr('id')]) tree_component.inst[jQueryKam(this).attr('id')].destroy();
	...
	function tree_component () {
		return {
			...
			settings : jQueryKam.extend({},jQueryKam.tree.defaults),

			init : function(elem, conf) {
				var _this = this;
				this.container = jQueryKam(elem);
				if(this.container.size == 0) return false;
				tree_component.inst[this.cntr] = this;
				if(!this.container.attr("id")) this.container.attr("id","jstree_" + this.cntr); 
				tree_component.inst[this.container.attr("id")] = tree_component.inst[this.cntr];
				tree_component.focused = this.cntr;
				this.settings = jQueryKam.extend(true, {}, this.settings, conf);
				...
				if(this.settings.ui.theme_name !== false) {
					if(this.settings.ui.theme_path === false) {
						jQueryKam("script").each(function () { 
					...
					if(this.settings.ui.theme_path != "" && jQueryKam.inArray(this.settings.ui.theme_path, tree_component.themes) == -1) {
				...
				if(tree_component.drag_drop.marker === false) tree_component.drag_drop.marker = jQueryKam("<div>").attr({ id : "jstree-marker" }).hide().appendTo("body");
			...
			refresh : function (obj) {
				...
				if(this.settings.opened != false) {
					jQueryKam.each(this.settings.opened, function (i, item) {
				...
				if(this.selected) {
					...
					if(obj) {
						jQueryKam(obj).find("li:has(a.clicked)").each(function () {
							if(this.id) _this.settings.selected.push("#" + this.id);
						});
					}
					else {
						if(this.selected_arr) {
							jQueryKam.each(this.selected_arr, function () {
					...
					if((typeof this.settings.selected).toLowerCase() == "object") {
						jQueryKam.each(this.settings.selected, function () {
				...
				var _this = this;
				var _datastore = new jQueryKam.tree.datastores[this.settings.data.type]();
				...
				_datastore.load(this.callback("beforedata",[false,this]),this,this.settings.data.opts,function(data) {
					...
					_datastore.parse(data,_this,_this.settings.data.opts,function(str) {
						...
						_this.container.empty().append(jQueryKam("<ul class='ltr'>").html(str));
			...
			reselect : function (is_callback) {
				...
				if(this.settings.selected !== false) {
					jQueryKam.each(this.settings.selected, function (i) {
						if(_this.is_partial_refresh)	_this.select_branch(jQueryKam(_this.settings.selected[i].toString().replace('/','\\/'), _this.container), (_this.settings.rules.multiple !== false) );
						else							_this.select_branch(jQueryKam(_this.settings.selected[i].toString().replace('/','\\/'), _this.container), (_this.settings.rules.multiple !== false && i > 0) );
					});
			...
			get : function (obj, format, opts) {
				...
				return new jQueryKam.tree.datastores[format]().get(obj, this, opts);
			},
			...
			attach_events : function () {
				...
				jQueryKam("li", this.container.get(0))
					.live("click", function(event) { // WHEN CLICK IS ON THE ARROW
						if(event.target.tagName != "LI") return true;
						_this.off_height();
						if(event.pageY - jQueryKam(event.target).offset().top > _this.li_height) return true;
						_this.toggle_branch.apply(_this, [event.target]);
						event.stopPropagation();
						return false;
					});
				jQueryKam("a", this.container.get(0))
					.live("click", function (event) {
						...
						event.target.blur();
            //_this.select_branch.apply(_this, [event.target, event.ctrlKey || _this.settings.rules.multiple == "on"]);
            return true; // dodane, żeby użyło hrefa
					...
					.live("mousedown", function (event) {
						...
						if(_this.settings.rules.multiple != false && _this.selected_arr.length > 1 && obj.children("a:eq(0)").hasClass("clicked")) {
							...
							if(counter > 0) {
								...
								tree_component.drag_drop.isdown		= true;
								tree_component.drag_drop.drag_help	= jQueryKam("<div id='jstree-dragged' class='tree " + ( _this.settings.ui.theme_name != "" ? " tree-" + _this.settings.ui.theme_name : "" ) + "' />").append("<ul class='" + _this.container.children("ul:eq(0)").get(0).className + "' />");
			...
			off_height : function () {
				if(this.offset === false) {
					...
					var tmp = 0;
					tmp = parseInt(jQueryKam.curCSS(this.container.get(0), "paddingTop", true),10);
					if(tmp) this.offset.top += tmp;
					tmp = parseInt(jQueryKam.curCSS(this.container.get(0), "borderTopWidth", true),10);
					if(tmp) this.offset.top += tmp;
			...
			get_node : function (obj) {
				return jQueryKam(obj).closest("li");
			},
			...
			check_move : function (nod, ref_node, how) {
				if(this.locked) return false;
				if(jQueryKam(ref_node).closest("li.dragged").size()) return false;

				var tree1 = nod.parents(".tree:eq(0)").get(0);
				var tree2 = ref_node.parents(".tree:eq(0)").get(0);
				// if different trees
				if(tree1 && tree1 != tree2) {
					var m = jQueryKam.tree.reference(tree2.id).settings.rules.multitree;
					if(m == "none" || (jQueryKam.isArray(m) && jQueryKam.inArray(tree1.id, m) == -1)) return false;
				}
				...
				if(typeof r.valid_children != "undefined" && (r.valid_children == "none" || (typeof r.valid_children == "object" && jQueryKam.inArray(nod_type, jQueryKam.makeArray(r.valid_children)) == -1))) return false;
			...
			deselect_branch : function (obj) {
				...
				if(this.settings.rules.multiple != false && this.selected_arr.length > 1) {
					...
					this.container.find("a.clicked").filter(":first-child").parent().each(function () {
						_this.selected_arr.push(jQueryKam(this));
					});
				...
				if(this.settings.data.async && obj.find("li").size() == 0) {
					...
					_datastore.load(this.callback("beforedata",[obj,this]),this,this.settings.data.opts,function(data){
						...
						_datastore.parse(data,_this,_this.settings.data.opts,function(str){
							...
							obj.children("ul:eq(0)").replaceWith(jQueryKam("<ul>").html(str));
			...
			set_lang : function (i) { 
				if(!jQueryKam.isArray(this.settings.languages) || this.settings.languages.length == 0) return false;
				if(this.locked) return this.error("LOCKED");
				if(!jQueryKam.inArray(i,this.settings.languages) && typeof this.settings.languages[i] != "undefined") i = this.settings.languages[i];
			...
			create : function (obj, ref_node, position) { 
				...
				var torename = false; 
				if(!obj)	obj = {};
				else		obj = jQueryKam.extend(true, {}, obj);
				...
				var obj_s = jQueryKam.tree.datastores.json().parse(obj,this);
				obj_s = this.callback("onparse", [obj_s, this]);
				var $li = jQueryKam(obj_s);
				...
				if(typeof r.valid_children != "undefined" && (r.valid_children == "none" || (jQueryKam.isArray(r.valid_children) && jQueryKam.inArray(nod_type, r.valid_children) == -1))) return this.error("CREATE: NODE NOT A VALID CHILD");
			...
			rename : function (obj, new_name) {
				...
				if((typeof new_name).toLowerCase() == "string") {
					...
					_this.inp = jQueryKam("<input type='text' autocomplete='off' />");
					_this.inp
					...
					var spn = jQueryKam("<span />").addClass(obj.attr("class")).append(icn).append(_this.inp);
					...
					if(obj.size()) {
						...
						if(obj.find("a.clicked").size()) {
							...
							this.container.find("a.clicked").filter(":first-child").parent().each(function () {
								...
								if(jQueryKam(this).parents().index(obj) != -1) return true;
								_this.selected_arr.push(jQueryKam(this));
							});
			...
			callback : function (cb, args) {
				...
				for(var i in this.settings.plugins) {
					if(typeof jQueryKam.tree.plugins[i] != "object") continue;
					p = jQueryKam.tree.plugins[i];
			...
			moved : function (what, where, how, is_new, is_copy, rb) {
				var what	= jQueryKam(what);
				var $parent	= jQueryKam(what).parents("ul:eq(0)");
				var $where	= jQueryKam(where);
				...
				if(is_copy) {
					...
					_what.each(function (i) {
						...
						jQueryKam(this).find("li").each(function () {
							this.id = this.id + "_copy";
						});
						jQueryKam(this).removeClass("dragged").find("a.clicked").removeClass("clicked").end().find("li.dragged").removeClass("dragged");
					});
				}
				else _what = what;
				if(is_new) {
			...
			search : function(str, func) {
				...
				if(this.settings.data.async) {
					if(!this.srch_opn) {
						var dd = jQueryKam.extend( { "search" : str } , this.callback("beforedata", [false, this] ) );
						jQueryKam.ajax({
							...
							success		: function (data) {
								_this.srch_opn = jQueryKam.unique(data.split(","));
			...
			destroy : function() {
				...
				jQueryKam("#" + this.container.attr("id")).die("click.jstree").die("dblclick.jstree").die("mouseover.jstree").die("mouseout.jstree").die("mousedown.jstree");
	...
	tree_component.mouseup = function(event) {
		...
		if(tmp.moving && jQueryKam.tree.drag_end !== false) jQueryKam.tree.drag_end.call(null, event, tmp);
	...
	tree_component.mousemove = function(event) {
		...
		if(tmp.isdown) {
			...
			if(tmp.drag_help !== false) {
				if(!tmp.appended) {
					if(tmp.foreign !== false) tmp.origin_tree = jQueryKam.tree.focused();
					jQueryKam("body").append(tmp.drag_help);
			...
			if(is_start && jQueryKam.tree.drag_start !== false) jQueryKam.tree.drag_start.call(null, event, tmp);
			if(jQueryKam.tree.drag !== false) jQueryKam.tree.drag.call(null, event, tmp);

			if(event.target.tagName == "DIV" && event.target.id == "jstree-marker") return false;

			var et = jQueryKam(event.target);
			...
			if(event.target.tagName == "A" || event.target.tagName == "INS") {
				...
				jQueryKam.each(arr, function(i, val) {
				...
				if(ok) {
					...
					tmp.move_type	= mov;
					tmp.ref_node	= jQueryKam(event.target);
	...
	jQueryKam(function () { 
		jQueryKam(document).bind("mousemove.jstree",	tree_component.mousemove); 
		jQueryKam(document).bind("mouseup.jstree",		tree_component.mouseup); 
	});
	...
	jQueryKam(function () {
		...
		if(jQueryKam.browser.msie) { 
			if(jQueryKam.browser.version == 6) css += '.tree li { height:18px; zoom:1; } .tree li li { overflow:visible; } .tree .ltr li.last { margin-top: expression( (this.previousSibling && /open/.test(this.previousSibling.className) ) ? "-2px" : "0"); } .marker { width:45px; background-position:-32px top; } .marker_plus { width:5px; background-position:right top; }';
			if(jQueryKam.browser.version == 7) css += '.tree li li { overflow:visible; } .tree .ltr li.last { margin-top: expression( (this.previousSibling && /open/.test(this.previousSibling.className) ) ? "-2px" : "0"); }';
		}
		if(jQueryKam.browser.opera) css += '.tree > ul > li.last:after { content:"."; display: block; height:1px; clear:both; visibility:hidden; }';
		if(jQueryKam.browser.mozilla && jQueryKam.browser.version.indexOf("1.8") == 0) css += '.tree .ltr li a { display:inline; float:left; } .tree li ul { clear:both; }';
...
(function (jQueryKam) {
	jQueryKam.extend(jQueryKam.tree.datastores, {
		"html" : function () {
			return {
				get		: function(obj, tree, opts) {
					return obj && jQueryKam(obj).size() ? jQueryKam('<div>').append(tree.get_node(obj).clone()).html() : tree.container.children("ul:eq(0)").html();
				},
		...
		"json" : function () {
			return {
				get		: function(obj, tree, opts) { 
					var _this = this;
					if(!obj || jQueryKam(obj).size() == 0) obj = tree.container.children("ul").children("li");
					else obj = jQueryKam(obj);
				...
				parse	: function(data, tree, opts, callback) { 
					...
					if(tree.settings.languages.length) {
						...
						var attr = {};
						attr["href"] = "";
						if(data.data.href) attr["href"]=data.data.href;
						attr["style"] = "";
						attr["class"] = "";
