			...
			return {
				get		: function(obj, tree, opts) {
					...
					if(obj.size() > 1) {
						...
						var str	 = '<root>';
						obj.each(function () {
							str += _this.get(this, tree, jQueryKam.extend(true, {}, opts, { callback : true }));
						});
						str		+= '</root>';
						return str;
					}
				...
				parse	: function(data, tree, opts, callback) { 
					...
					processor.importStylesheet(jQueryKam.tree.datastores.xml_nested.xsl);

					var result = jQueryKam((new XMLSerializer()).serializeToString(processor.transformToDocument(data)).replace('<?xml version="1.0"?>',''));
					// Opera bug
