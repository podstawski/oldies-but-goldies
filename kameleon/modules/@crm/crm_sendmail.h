...
function crm_encode($_str,$iso)
{

	$_str = crm_win2iso($_str);
...
function crm_sendmail($obj)
{
	...
	$msg = stripslashes(crm_encode($msg,0));

	$subject = stripslashes($subject);
	$subject = crm_encode($subject,1);

	$from = crm_encode($from,1);
	...
	if (is_Array($cc))
	{
		...
		{
			$mailcc.="Cc: ".crm_encode($cc[$c],1)."\n";
		}
	}
	else if (strlen($cc)) $mailcc ="Cc: ".crm_encode($cc,1)."\n";
...
//	$msgbody.= "--".$boundary."\n";
	...
	{
		$bccpole.="Bcc: ".crm_encode($bcc[$i],1)."\n";
