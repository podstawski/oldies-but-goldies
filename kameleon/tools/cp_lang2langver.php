<?php
	...
	if ($deletebefore)
	{
		$query="DELETE FROM webpage WHERE server=$server AND lang='$dst' AND ver=$ver;
				DELETE FROM weblink WHERE server=$server AND lang='$dst' AND ver=$ver;
				DELETE FROM webtd WHERE server=$server AND lang='$dst' AND ver=$ver";
		parse_str(ado_query2url($query));
	}
	$query="SELECT count(*) AS c1 FROM webpage WHERE server=$server AND lang='$dst' AND ver=$ver";
	parse_str(ado_query2url($query));
	$query="SELECT count(*) AS c2 FROM weblink WHERE server=$server AND lang='$dst' AND ver=$ver";
	parse_str(ado_query2url($query));
	$query="SELECT count(*) AS c3 FROM webtd WHERE server=$server AND lang='$dst' AND ver=$ver";
	parse_str(ado_query2url($query));

	if ($c1+$c2+$c3) 
	{
		echo "W wersji jezykowej '$dst' sa wpisy ($c1+$c2+$c3)\n";
		return;
	}
	...
	$query="
		...
		SELECT server,id,ver,'$dst',title,description,keywords,
			...
			nositemap,noproof,title_short,accesslevel
		FROM webpage WHERE server=$server AND lang='$src' AND ver=$ver;
		...
		FROM webtd WHERE server=$server AND lang='$src' AND ver=$ver;
		...
		SELECT server,page_id,menu_id,ver,'$dst',img,imga,alt,alt_title,page_target,href,
					pri,fgcolor,type,class,variables,name,hidden,target,lang_target,accesslevel,submenu_id
		FROM weblink WHERE server=$server AND lang='$src' AND ver=$ver;
		";

	
	$query=kameleon_copy_query('webpage',
				array(	'lang'=>"'$dst'",'nd_create'=>time(),'nd_update'=>time()),
				array( 'lang'=>"'$src'",'server'=>$server,'ver'=>$ver)
				);
	...
	function usage($me)
	{
		echo "$me server=xx src=yy dst=zz ver=x [deletebefore=1]\n";
		exit ();
	}