 ...
 *   $memcache = ...;
 *   $helper = new Auth_OpenID_GoogleDiscovery($consumer, array('/etc/ssl/certs'), $memcache);
 *   // Use consumer
 *   $auth_request = $consumer->begin(...);
 */ 
class Auth_OpenID_GoogleDiscovery {
     ...
     */
    function Auth_OpenID_GoogleDiscovery($consumer, $trust_roots = null, $memcache = null) {
        $this->verifier = new Auth_OpenID_SimpleSign($trust_roots);
    ...
    function &fetch_xrds_services($authority, $url, $fetcher, $use_cache = true) {
        ...
        if ($body == null) {
            ...
            if ($signed_by != strtolower($authority) && $signed_by != Auth_OpenID_GoogleDiscovery::HOSTED_ID) {
    ...
    function get_user_xrds_url($xrds, $claimed_id) {
        $types_to_match = array(Auth_OpenID_GoogleDiscovery::DESCRIBED_BY_TYPE);
        foreach(@$xrds->services() as $service) {
            if ($service->matchTypes($types_to_match)) {
                $url = null;
                $authority = null;
                $elements = $service->getElements(Auth_OpenID_GoogleDiscovery::URI_TEMPLATE_ELEMENT);
                if ($elements != null) {
                    $template = $elements[0]->textContent;
                    $url = str_replace(Auth_OpenID_GoogleDiscovery::USER_URI_VAR, urlencode($claimed_id), $template);
                }
                $elements = $service->getElements(Auth_OpenID_GoogleDiscovery::NEXT_AUTHORITY_ELEMENT);
                if ($elements != null) {
    ...
    function get_cache($key) {
        ...
        return $this->memcache->get(Auth_OpenID_GoogleDiscovery::CACHE_PREFIX.$key);
    }
    ...
    function put_cache($key, $value) {
        if ($this->memcache != null) {
            $this->memcache->set(Auth_OpenID_GoogleDiscovery::CACHE_PREFIX.$key, $value, 0, Auth_OpenID_GoogleDiscovery::CACHE_EXPIRY);
        }
    }
}
...
class Auth_OpenID_SimpleSign {
     ...
     */
    function Auth_OpenID_SimpleSign($trust_roots = null) {
    ...
    function get_xpath($doc) {
        $xp = new DOMXPath($doc);
        $xp->registerNamespace("ds", Auth_OpenID_SimpleSign::NS_DSIG);
        $xp->registerNamespace("xrds", Auth_OpenID_SimpleSign::NS_XRDS);
        return $xp;
    }
