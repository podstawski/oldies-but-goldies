...
class AuthController extends AbstractController
{
     ...
     * @var array
     */
    protected $_oauthScopes,$_oauthScopes2;
	...
	$this->_oauthScopes2 = Zend_Registry::get('oauth_scopes2');
	...
	protected function checkDomain()
	{
		if (!isset($this->_auth->DOMAIN['id'])) {
			...
			if ((!$domain->marketplace) and (!@$_SESSION['force-personal-domain']) and (!$domain->isSpecial()) and empty($domain->oauth_token) and (empty($this->user) or empty($this->user->token))) {
	...
	protected function checkUser2()
	{
		...
		if ($user == null) {
			...
			$user->role = $modelUsers::ROLE_TEACHER;
		...
		if ($user->getDomain()->isSpecial() or (@$_SESSION['force-personal-domain'])) {
	...
	$oauthScopes = $googleapps['oauth2'] ? $this->_oauthScopes2 : $this->_oauthScopes;
	
        if (empty($namespace)) {
            assert(!is_array($oauthScopes));
            $scopes = $oauthScopes ;
        } else {
            assert(is_array($oauthScopes));
            $scopes = $oauthScopes[$namespace];
        }

	if ($googleapps['oauth2'])
	{
	    $token=GN_Goauth2::getAccessToken($googleapps['clientId'],$googleapps['clientSecret'],$scopes,$this->view->absoluteUrl(array('controller'=>'auth','action'=>'open-id'),null,true));
	    return $token;
	}
