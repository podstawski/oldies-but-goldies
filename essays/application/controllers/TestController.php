...
class TestController extends AbstractController {
	...
	protected function getClient($mode = GN_GClient::MODE_AUTO)
	{

		$token=$this->user->getAccessToken();
		$domain=$this->user->getDomain();
	
		return (is_object($token) && $token->oauth2 && !$domain->marketplace) ? new GN_GClient2($this->user, $mode) : new GN_GClient($this->user, $mode);		
	}
	...
	protected function getAllUsers($client)
	{
		
		$cacheManager = Zend_Registry::get('cache');
		$cacheKey='all_users_of_the_domain_'.md5($this->user->getDomain()->domain_name);
		
	
		$allusers = $cacheManager->load($cacheKey);

		if ($allusers === false) {
			$allusers = $client->retrieveAllUsers();
			$cacheManager->save($allusers, $cacheKey);
		}
		
		return $allusers;
	}
	
	protected function storeCacheUserName($email,$name)
	{
		$cacheKey='cache_username_'.md5($email);
		$cacheManager = Zend_Registry::get('cache');
		$cacheManager->save($name, $cacheKey);
		return $name;
	}
	
	protected function getCacheUserName($email)
	{
		$cacheManager = Zend_Registry::get('cache');
		$cacheKey='cache_username_'.md5($email);
		$name=$cacheManager->load($cacheKey);
		
		if (!$name && !$this->user->getDomain()->isSpecial() && ($this->user->getDomain()->marketplace || !empty($this->user->getDomain()->oauth_token)) )
		{
			$client=$this->getClient(GN_GClient::MODE_DOMAIN);
			$users = $this->getAllUsers($client);
			$id = $this->constructId($email);
			...
			if ($name) $this->storeCacheUserName($email,$name);
		...
		if ($groupID !== null) {
			...
			foreach ($members as $member) {
				...
				if ($detailLevel >= 2) {
					...
					$this->storeCacheUserName($mail,$name);
					$folk['name'] = $name;
				}
	...
	public function editContentsAction() {
		...
		try {
			...
			$ep->request($client, 'create-document (creating document: ' . $meta['document-title'] . ')', function($c) use (&$meta) {
				...
				$meta['document-id'] = is_object($documentID)?$client->getDocumentIdFromObj($documentID):$documentID;
			}, null, function($c) {
				GN_Debug::debug($c['exceptions']);
			});
			...
			$ep->request($client, 'create-document (adding ' . $meta['document-title'] . ' document to ' . $meta['document-title'] . ' folder)', function($c) use (&$meta) {
				$client = $c['client'];
				$client->putItemToFolder($meta['document'],$meta['folder-uri']);
			}, null, function($c) {
				GN_Debug::debug($c['exceptions']);
			});
			...
			$ep->request($client, 'create-document (adding ' . $meta['document-title'] . ' folder to root folder)', function($c) use (&$meta) {
				$client = $c['client'];
				$client->putItemToFolder($meta['folder'], $meta['main-folder-uri']);
			}, null, function($c) {
				GN_Debug::debug($c['exceptions']);
			});
			...
			$ep->request($client, 'create-document (removing ' . $meta['document-title'] . ' folder from root folder)', function($c) use (&$meta) {
				$client = $c['client'];
				$client->deleteItemFromRootFolder($meta['document-id']);
			}, null, function($c) {
				GN_Debug::debug($c['exceptions']);
			});
			...
			$ep->request($client, 'create-test (removing ' . $meta['document-title'] . ' folder from root folder)', function($c) use (&$meta) {
				...
				$client->deleteItemFromRootFolder(end($tmp));
			}, null, function($c) {
				GN_Debug::debug($c['exceptions']);
			});
			...
			if ($this->_hasParam('delete-documents')) {
				...
				$ep->request($client, 'delete-test (removing test folder)', function($c) use (&$meta, $test) {
					if (!empty($test->folder_uri)) {
						$client = $c['client'];
						$client->deleteFolder($test->folder_uri);
					}
				});
				if (!$ep->work()) {
	...
	protected function userIfCli($user_id)
	{
		$role = Zend_Controller_Front::getInstance()->getPlugin('GN_Plugin_Acl')->getRoleName();
		$modelUsers = new Model_Users();
		if ($role==Model_Users::ROLE_CLI)
		{
			$this->user=$modelUsers->fetchRow($modelUsers->select()->where('id='.$user_id));
		}		
	}
	...
	private function doOpenTest($testID) {
		...
		try {
			...
			foreach ($participants as $participant) {
				...
				$meta[$participant->id]['email']=$participant->participant_email;
				
				if (!empty($participant->document_id)) {
					$meta[$participant->id]['document-copy-id'] = $participant->document_id;
					$meta[$participant->id]['document-copy'] = $client->getDocumentEntry($participant->document_id);
					...
					$ep->request($client, 'open-test (duplicating document for ' . $participant->participant_email . ')', function($c) use (&$meta, $test, $participant) {
						...
						$doc=$client->copyDocument($test->document_id, $meta[$participant->id]['document-copy-title']);

						
						if (!empty($doc))
						{
							$meta[$participant->id]['document-copy'] = $doc;
							$meta[$participant->id]['document-copy-id'] = $doc->id;
						}
			...
			$shouldWork = false;
			foreach ($participants as $participant) {
				if (!empty($participant->document_id)) continue;
				$shouldWork=true;
				$ep->request($client, 'open-test (fetching document for ' . $participant->participant_email . ')', function($c) use (&$meta, $test, $participant, &$_this) {
					...
					{
						$documentCopy = $client->getDocumentByTitle($meta[$participant->id]['document-copy-title']);				
			...
			foreach ($participants as $participant) {
				...
				if ($participant->participant_email != $user->email) {
					$ep->request($client, 'open-test (changing permission for ' . $participant->participant_email . ')', function($c) use ($meta, $participant) {
						...
						$client->addPermisionToObj($meta[$participant->id]['document-copy'],$meta[$participant->id]['email'],'writer');
					}, null,
				...
				$ep->request($client, 'open-test (adding document for ' . $participant->participant_email . ' to test folder)', function($c) use (&$meta, $test, $participant) {
					$client = $c['client'];
					$client->putItemToFolder($meta[$participant->id]['document-copy'],$test->folder_uri);
				...
				$ep->request($client, 'open-test (removing document for ' . $participant->participant_email . ' from root folder)', function($c) use (&$meta, $participant) {
					...
					$client->deleteItemFromRootFolder($documentCopyID);
			...
			foreach ($participants as $participant) {
					...
					$ep->request($client, 'open-test (changing writers must not share ' . $participant->participant_email . ')', function($c) use ($meta, $participant) {
						$client = $c['client'];
						$documentCopy = $meta[$participant->id]['document-copy'];
						$client->writersCanShare($documentCopy,false);
					}, null,
					function($c) use (&$_this) {
						throw new Exception($_this->view->translate('open_test_document_permission_change_error'));
					});					
			...
			foreach ($participants as $participant) {
				...
				$ep->request($client, 'open-test (changing share permission for writers)', function($c) use ($meta, $participant) {
					$client = $c['client'];
					$client->anyOneWithLinkCanEdit($meta[$participant->id]['document-copy-id']);
		...
		try {
			...
			foreach ($participants as $participant) {
				...
				$ep->request($client, 'close-test (changing permissions of document for ' . $participant->participant_email . ')', function($c) use (&$meta, $participant, $user) {
					...
					$publishedRaw = $client->getFilePublishDate($documentCopy);
					$updatedRaw = $client->getFileUpdateDate($documentCopy);

					//skonwertowanie strtotime spowoduje automatyczna konwersje z UTC na strefe serwera. probably.
					$publishedUnix = strtotime($publishedRaw);
					$updatedUnix = strtotime($updatedRaw);


					$author=$client->getFileUpdateAuthor($documentCopy);
					$owner=$client->getFileOwner($documentCopy);
					if ($author != $owner) {
					...
					if ($participant->participant_email != $user->email) {
						$client->revokeAllPermisions($meta[$participant->id]['document-copy'],$participant->participant_email);
			...
			foreach ($participants as $participant) {
				...
				$ep->request($client, 'close-test (changing permissions of document for ' . $participant->participant_email . ')', function($c) use (&$meta, $participant, $user) {
					...
					if ($participant->participant_email != $user->email) {
						$client->addPermisionToObj($documentCopy,$participant->participant_email,'reader','commenter', false);
			...
			foreach ($participants as $participant) {
				...
				$ep->request($client, 'close-test (changing permissions of document for ' . $participant->participant_email . ')', function($c) use (&$meta, $participant, $user) {
					$client = $c['client'];			
					$client->anyOneWithLinkCanEdit($meta[$participant->id]['document-copy-id'],'reader','commenter');
			...
			if (!$test) {
				...
				foreach ($emails as $email) {
					...
					if (!$this->isValidEmail($email)) {
						...
						$name=$this->getCacheUserName($email);
						if ($name) {
	...
	public function ajaxCheckTitleAction() {
		...
		try {
			...
			$documentID = $test->document_id;
			$folderID = $test->folder_uri;
			try {
				$client = $this->getClient();
				$json['oauth2']=@$client->oauth2;

				$document = $client->getDocumentEntry($documentID);
				
				if (!empty($document) and !empty($document->title)) {
					$documentTitle = is_string($document->title)? $document->title:$document->title->text;
				} else {
					$documentTitle = $test->document_title;
				}
				...
				if (!empty($folderID) )
				{
					$client->updateFolder($folderID, array('title'=>$documentTitle));
				}

				$json['title'] = $documentTitle;
				if ($test->document_title != $documentTitle)
				{
	...
	public function ajaxCheckParticipantModDateAction() {
		...
		try {
			...
			if ($test->status != Model_Tests::STATUS_UNOPENED) {
				...
				$publishedRaw = $client->getFilePublishDate($document);
				$updatedRaw = $client->getFileUpdateDate($document);
				...
				$author=$client->getFileOwner($document);
				$writer=$client->getFileUpdateAuthor($document);
				

				if ($author != $writer) {
					$x=$updatedUnix - $publishedUnix;
		...
		if ($this->_hasParam('search-for-users')) {
			...
			foreach ($tests as $test) {
				...
				foreach ($participants as $participant) {
					...
					if (!empty($participant->participant_name)) $this->storeCacheUserName($participant->participant_email,$participant->participant_name);
				}
			}
		}
		...
		if ($this->_hasParam('search-for-groups')) {
			...
			if (!$this->user->getDomain()->isSpecial() and ($this->user->getDomain()->marketplace or !empty($this->user->getDomain()->oauth_token))) {
				try {
					...
					if ($groups === false) {
						$googleGroups = array();
						$clientDomain=$this->getClient(GN_GClient::MODE_DOMAIN);
					...
					foreach ($groups as $group) {
						if (empty($filter) or preg_match('/(^|\W)' . $filter . '/i', strtolower($group['name'])) or preg_match('/(^|\W)' . $filter . '.*@/i', strtolower($group['e-mail']))) {
							...
							if ($subfolks === false) {
								$subfolks = $this->retrieveFolks($group['id'], 2);
