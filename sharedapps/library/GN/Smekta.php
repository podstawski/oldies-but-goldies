...
class GN_Smekta
{
	...
	if (!strstr($txt,"\n")) {
		...
		foreach ($dir AS $d) {
		    if ($d && substr($txt,0,1)!='/') $path="$d/$txt";
		    else $path=$txt;
	...
	foreach ($vars AS $k=>$v) {
	    ...
	    if (is_array($v) && $k!='GLOBALS') self::clearvars($vars[$k]);
	}
	...
	if (!self::$unique_debug_id) self::$unique_debug_id=time()-rand(1,10000);
	else self::$unique_debug_id++;
	...
	if ($part && isset($vars[$part])) {
	    $ret.='<pre style="font-size:10px; font-family: Courier">'.print_r($vars[$part],1).'</pre>';
    ...
    protected function _debugger($debug_id,$txt=null)
    {
