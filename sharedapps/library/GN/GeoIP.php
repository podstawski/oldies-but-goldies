 ...
 * @link     http://pear.php.net/package/GN_GeoIp
 ...
 *
 * <code>
 * $geoip = GN_GeoIP::getInstance('/path/to/geoipdb.dat', GN_GeoIP::SHARED_MEMORY);
 * </code>
 *
...
class GN_GeoIP
{
     ...
     */
    const ERR_INVALID_IP =  218624992; // crc32('GN_GeoIP::ERR_INVALID_IP')

    /**
     * Exception error code when there is a DB-format-related error.
     */
    const ERR_DB_FORMAT = 866184008; // crc32('GN_GeoIP::ERR_DB_FORMAT')
    ...
    */
     ...
     * $geoip = GN_GeoIP::getInstance('C:\mydb.dat', GN_GeoIP::MEMORY_CACHE);
     * ....
     *
     * // later in code, request instance with no flags specified.
     * $geoip = GN_GeoIP::getInstance('C:\mydb.dat');
     *
    ...
    public static function getInstance($filename = null, $flags = null)
    {
        ...
        if (!isset(self::$instances[$filename])) {
            self::$instances[$filename] = new GN_GeoIP($filename, $flags);
        }
     ...
     * @param string $addr IP address (hostname not allowed).
     *
     * @return GN_GeoIP_Location The full location record.
     *
     * @throws Exception - if IP address is invalid.
     */
    public function lookupLocation($addr)
    {
        include_once dirname(__FILE__) . '/GeoIP/Location.php';
        $ipnum = ip2long($addr);
        if ($ipnum === false) {
            throw new Exception("Invalid IP address: " . var_export($addr, true), self::ERR_INVALID_IP);
        }
    ...
    /**
     * Seek and populate GN_GeoIP_Location object for converted IP addr.
     * Note: this
     *
     * @param int $ipnum Converted IP address.
     *
     * @return GN_GeoIP_Location
     */
    ...
    public static function getCountryCode($ip=null) {
        $geoip = self::getInstance(dirname(__FILE__) . '/GeoIP/GeoIP.dat');

        if (!$ip) $ip=isset($_SERVER['HTTP_X_FORWARDED_FOR']) ? $_SERVER['HTTP_X_FORWARDED_FOR'] : $_SERVER['REMOTE_ADDR'];
        $location = $geoip->lookupLocation($ip);
        
        
        return  isset($location->countryCode) ? $location->countryCode : 'PL';
    }
}
