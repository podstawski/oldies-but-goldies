...
class Bootstrap extends GN_Bootstrap
{
     ...
     * @var string
     */
    protected $_sessionNamespace = 'Competence';
    ...
    public function _initLanguage()
    {
        ...
        if (isset($_GET['lang']) && $adapter->isAvailable($_GET['lang'])) {
            $session->language = $_GET['lang'];
		} elseif ($user and $user->language and $adapter->isAvailable($user->language)) {
			$session->language = $user->language;
        } elseif (isset($session->preferredLanguage) && $adapter->isAvailable($session->preferredLanguage)) {
