...
class GN_Game_Socket {
	...
	*/
	...
	function wsAddClient($socket, $clientIP) {
		...
		$this->wsClients[$clientID] = array("Socket"=>$socket,
		"MessageBuffer"=> '',
	...
	function wsRemoveClient($clientID) {
		...
		$closeStatus = $this->wsClients[$clientID]['CloseStatus'];
		if ($this->wsClients[$clientID]['Admin']===true)
		{
			if ( array_key_exists('admin_close', $this->wsOnEvents) )
			foreach ( $this->wsOnEvents['admin_close'] as $func )
		...
		$clientIP = $this->wsClients[$clientID]['IP'];
	...
	function wsAdminProcess($clientID,$buffer,$bufferLength)
	{	
		...
		if ($json!==null)
		{
			if ($this->wsClients[$clientID]['AKey']!='')
			{
				if ($json['Register']!='')
				{
					
					foreach ($this->registered as $iregister => $register)
					{
						if ($register['Key'] == $json['Register']['Key'])
						{
							$this->wsClients[$clientID]['rid']=$iregister;
							return true;
						}
					}
					$this->registered[] = array();
					$rid = count($this->registered)-1;
					$this->registered[$rid]['Name'] = $json['Register']['Name'];
					$this->registered[$rid]['Key'] = $json['Register']['Key'];
					$this->registered[$rid]['Mail'] = $json['Register']['Mail'];
					$this->wsClients[$clientID]['rid']=$rid;
					//print_r($this->wsClients[$clientID]);
					echo "User: ".$json['Register']['Name'] ." registered.\n";
				}
			}
			elseif ($json['Authorize']!='')
			{
				//echo "Authorize nie jest pusty";			
				if ($json['Authorize'] == $this->adminKey)
				$this->wsClients[$clientID]['AKey']=$json['Authorize'];
				else
				{
	...
	function wsProcessClient($clientID, &$buffer, $bufferLength) {
		...
		if ($this->wsClients[$clientID]['ReadyState'] == self::WS_READY_STATE_OPEN) {
			// handshake completed
			if ($this->wsClients[$clientID]['Admin'] ===false)
			{
			...
			else
			{
				...
				$result = $this->wsAdminProcess($clientID,$buffer,$bufferLength);
				//var_dump($result);
			}
		...
		elseif ($this->wsClients[$clientID]['ReadyState'] == self::WS_READY_STATE_CONNECTING) {
			...
			if ($result) {
				...
				if ($this->wsClients[$clientID]['Admin']===true)
				{
					if ( array_key_exists('admin_open', $this->wsOnEvents) )
					foreach ( $this->wsOnEvents['admin_open'] as $func )
					$this->$func($clientID);
				}else
				{			
	...
	function wsProcessClientHandshake($clientID, &$buffer) {
		...
		for ($i=1; $i<$headersCount; $i++) {
			...
			$parts[0] = substr($headers[$i], 0,strpos($headers[$i],':'));
			$parts[1] = substr($headers[$i], strpos($headers[$i],':')+1,strlen($headers[$i]));
		...
		
		{
			...
			//$this->wsClients[$clientID].push($headersKeyed);
			// build headers
			$headers = array(
			...
			$headers = implode("\r\n", $headers)."\r\n\r\n";
			$this->wsClients[$clientID]['Headers'] = $headersKeyed;
		...
		else
		{
			...
			if ($headersKeyed['Key']!='')
			{
				$key = $headersKeyed['Key'];
				$hash = base64_encode(sha1($key.'258EAFA5-E914-47DA-95CA-C5AB0DC85C11', true));
				$headers = array(
				'Key: '.$hash
				);
				...
				$this->wsClients[$clientID]['Admin'] = true;	
				return true;
			}
		}
	}
