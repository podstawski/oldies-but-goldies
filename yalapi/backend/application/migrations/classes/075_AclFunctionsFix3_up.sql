...
BEGIN
	...
	FOR result IN EXECUTE('SELECT  _' || "right" || ' FROM ' || table_name || '_acl WHERE object_id IN (0,' || object_id || ') AND username IN (''*'', ''' || CURRENT_DATABASE() || '_'|| username || ''') ORDER BY _'||"right"||' DESC LIMIT 1;') LOOP
	END LOOP;
